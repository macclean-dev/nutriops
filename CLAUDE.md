...
| Prioridade | Item |
|------------|------|
| 🔴 CRÍTICA (em andamento) | **Advisor: `tenants` publicamente acessível + colunas sensíveis** (`access_token`/`setup_pin_hash`). Fix pronto no código (v1.9.31): acesso anon à `tenants` migrado pra RPCs `security definer` (`get_tenant_by_token` não devolve o `access_token`) + `tenant-sync.js` com fallback. **FALTA você rodar o SQL** em `docs/security-tenants-lockdown.sql`: **Parte 1 agora** (cria RPCs, não tranca), e a **Parte 2 depois do deploy v1.9.31 + teste** (liga RLS deny-all + revoga grants → fecha os 2 alertas). Rollback é 1 comando. As 8 tabelas de dados seguem na Fase 3 (device-token). |
