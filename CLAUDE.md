...
| 🟢 Alta (épico · EM ANDAMENTO) | **Auth+RLS — Fase 3 (ligar RLS), rollout iniciado 18/07.** JÁ FEITO e validado em prod: `/admin` migrou pro Supabase Auth (v1.9.37, fecha o backdoor `VITE_ADMIN_PASSWORD`); RPCs `security-definer` gated por `app_metadata.role='admin'` — `admin_list_tenants` (lista tenants do Supabase, fonte da verdade) + `admin_recent_temperature_records` (HealthView) (v1.9.38); `app_metadata` (role/tenant_id) setado nas 3 contas device + admin (Parte A); 8 policies `tenant_isolation` aplicadas lendo `app_metadata` (Parte B — SEM bypass forjável de `user_metadata`, com exceção `__healthcheck__`); `VITE_DEVICE_PASSWORD` no Vercel — os 3 device-tokens validados na REST (JWT com tenant_id certo); **RLS LIGADO + VALIDADO na `special_controls`** (device escreve só o próprio tenant=201, cross-tenant=403, anon lê=[], testWrite ok). **FALTA:** ligar RLS nas outras 7 (`alter table X enable row level security;` — `special_controls` provou o mecanismo; `temperature_records` por ÚLTIMO + teste em device real). SQL pronto em `docs/rls-fase3-policies.sql` (Parte C). Rollback = `disable` 1 comando/tabela. Todos os caminhos de escrita do app passam `tenantId`→device-token (repository.js: push online + drain da fila linha 317). |
...
- **RLS: rollout em andamento (18/07).** `tenants` fechada por RPC+deny-all
  (v1.9.31); **`special_controls` com RLS LIGADO + validado**; as outras 7 tabelas
  de dados ainda OFF (ligar via `docs/rls-fase3-policies.sql` Parte C, com o dono
  presente). O sync usa device-token por tenant (`app_metadata.tenant_id`), não
  mais só a anon key. Ver a pendência do épico Auth+RLS acima. O `SUPABASE_SQL`
  em `repository.js` ainda mostra as policies antigas (`user_metadata`) + os
  `disable` — desatualizado; a fonte de verdade é `docs/rls-fase3-policies.sql`
  (sync do SUPABASE_SQL = tarefa de limpeza da Fase 4).
