...
| 🟡 Média | **Sync das regras de validade (v1.9.100): rodar `docs/validity-rules-sync.sql` antes do deploy.** Cria a tabela `validity_rules` (1 linha por loja) + RLS no padrão de 4 caminhos já em produção. Sem isso, a aba Regras (Validades e Estoque) continua só local — quem ajustar num device não afeta o que os outros veem. Motivo: nutricionista ajusta de casa, tablet da produção não pegava a mudança (achado do dono em 09/08). |
