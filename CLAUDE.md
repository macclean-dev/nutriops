...
| Prioridade | Item |
|------------|------|
| 🔴 Alta | **Limpar as planilhas BPF duplicadas de Swiss/Bäckerei/DBK.** A v1.9.133 estancou (o seed casa por categoria+título, não só por id), mas as cópias já criadas continuam nos devices — podem ser dezenas. Limpar exige decidir o que fazer com os registros presos em cada cópia: cada `form_record` aponta pro `formId` da cópia em que foi preenchido, então dedupe ingênuo esconde NC histórica da Central. Proposta: sobrevivente = cópia com mais registros; medir antes nos devices reais. Causa raiz: `TPL_*()` desses tenants usam `id: uid()` (`forms.jsx`) — se um dia forem trocados por ids fixos, precisa migrar o `formId` dos registros junto. |
