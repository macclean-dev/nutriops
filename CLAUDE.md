...
| Prioridade | Item |
|------------|------|
| 🟢 Baixa | **Sinal da "Bancada congelada — F.2" corrigido (17/08) — registro.** 8 leituras invertidas (11-14/08), corrigidas via `docs/fix-bancada-congelada-f2.sql` com trilha preservada (`original_value`/`corrected_by`). ACHADO: só a de 11/08 era falso alarme (18 → -18, volta a conforme). As outras 7 são **desvio real** — a bancada operou entre -2,7 e -9,4 por três dias seguidos (12-14/08) com faixa -22/-18, ou seja, não estava congelando. Estava escondido atrás do sinal errado. ⚠️ Duas coisas em aberto: (a) a faixa do equipamento mudou de -22/-18 pra -18/-12 a partir de 15/08 — confirmar se foi decisão técnica e não ajuste pra silenciar alerta; (b) as leituras novas (-11,2 / -11,5) ficam ACIMA de -12, desvio leve — a F.4 no mesmo período marcou -14,7, então vale checar a F.2 fisicamente. Os 7 desvios precisam de ação corretiva registrada (RDC 216) ou o check A1 da Prontidão os mantém pendentes.
...
| 🟢 Baixa | **Auto-backfill: escopo corrigido (v1.9.137).** Estava passando `activeTenants` (todas as lojas do device) em vez de `visibleTenants` (as que a sessão alcança) — uma sessão de loja única (login por e-mail) tentava empurrar as OUTRAS lojas, RLS recusava (42501, corretamente), o backfill nunca fechava `failed:0` e repetia a cada boot pra sempre. Sintoma real: CASA DOCE com POST 401 em loop no console (15/08, 23h). Bump pra `nutriops.autobackfill.v4` pra parar quem já estava preso no loop. Resolvido — item fica só de registro. |
| 🟢 Baixa | **As 4 migrações pendentes de Manutenção/Prontidão/Central de NC (`manutencao-sync.sql`, `compliance-docs-sync.sql`, `pops-capacitacao-sync.sql`, `corrective-actions-sync.sql`) já rodaram — confirmado 20/08.** Query de conferência do `docs/rls-policies.sql` (PARTE 4) rodada pelo dono: as 20 tabelas de tenant vieram `ok` nos 4 caminhos de RLS, incluindo as 10 dessas migrações. Nenhum módulo de evidência é local-only. |
